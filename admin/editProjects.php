...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['submit']) && isset($_POST['title']) && isset($_POST['link'])) {
        $title = $_POST['title'];
        $link = $_POST['link'];
        $img = $_FILES['img'] ?? null;
        
        $conn = Database::getConnect();
        $pro = Operations::getProImage();
        ...
        if (!empty($uploadedImagePath)) {
            unlink($pro['img']);
            $sql = "UPDATE `project` SET `title` = '$title', `img` = '$uploadedImagePath', `created_at` = NOW(), `links` = '$link' WHERE `id` = '$getID'";
        } else {
            $sql = "UPDATE `project` SET `title` = '$title', `created_at` = NOW(), `links` = '$link' WHERE `id` = '$getID'";
        }
                                            ...
                                            <div class="card-body">
                                                ...
                                                <form class="forms-sample" method="POST" enctype="multipart/form-data">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="title" class="form-control" placeholder="Title" value="<?= $pro['title']; ?>" required>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="exampleInputLink1">Link</label>
                                                        <input type="text" name="link" class="form-control" placeholder="Links" value="<?= $pro['links']; ?>" required>
                                                    </div>
                                                    <div class="form-group">
